...
export const NoteCard = ({
  ...
  return (
    <span
      className='note-card'
      tabIndex={0}
      style={{ left: noteCardLeft, overflowY: editMode ? 'hidden' : 'auto' }}
    >
      {editMode ? (
        <NoteCardTextArea text={updatedNoteText} setText={setUpdatedNoteText} />
      ) : (
      ...
      <span className='note-card-controls'>
        {editMode ? (
          <NoteCardControl
            ...
            onClick={() => {
              setEditMode(false);
              editNote(favoriteNews, setFavoriteNews, id, updatedNoteText);
            }}
          />
        ) : (
          <>
            ...
            <NoteCardControl
              text='Delete Note'
              onClick={() => deleteNote(favoriteNews, setFavoriteNews, id)}
            />
          </>
        )}
      </span>
